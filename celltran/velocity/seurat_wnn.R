...
# Updated to run on transition cells found by celltran - 04/25/25

library(Seurat)
library(Signac)

# read in expression and accessbility data
brain <- readRDS("/dfs3b/ruic20_lab/singlecell/jeanl/organoid/data/Chen/rds/merged.rds")
...
barcodes <- read.delim("/dfs3b/ruic20_lab/jeancl2/data/celltran/velocity/seurat_wnn/filtered_cells.txt", header = F, stringsAsFactors = F)$V1
...
write.table(nn_idx, "/dfs3b/ruic20_lab/jeancl2/data/celltran/velocity/seurat_wnn/nn_idx.txt", sep = ',', row.names = F, col.names = F, quote = F)
write.table(nn_dist, "/dfs3b/ruic20_lab/jeancl2/data/celltran/velocity/seurat_wnn/nn_dist.txt", sep = ',', row.names = F, col.names = F, quote = F)
write.table(nn_cells, "/dfs3b/ruic20_lab/jeancl2/data/celltran/velocity/seurat_wnn/nn_cells.txt", sep = ',', row.names = F, col.names = F, quote = F)

# save sessionInfo for reproducibility
writeLines(capture.output(sessionInfo()), "/dfs3b/ruic20_lab/jeancl2/data/celltran/velocity/seurat_wnn/sessionInfo.txt")
