...
# Zhen's cell cycle score code cleaned up and applied to the transition cells and non-transition cells
# 04/21/25 - Jean Li

adata = sc.read("/dfs3b/ruic20_lab/jeancl2/data/chen_ro_clean.h5ad")
...
for cat in adata.obs.transition_cell.cat.categories.tolist(): 
    temp = adata[adata.obs.transition_cell == cat]
    sc.pp.normalize_per_cell(temp)
    sc.pp.log1p(temp)

    sc.tl.score_genes(temp, gene_list=cell_cycle_genes)
    ...
    sc.pl.umap(
        ...
        title=f"{cat} Cell Cycle Score",
        vmax = 0.25,
        save = f"cell_cycle_score_{cat}.png"
    )
