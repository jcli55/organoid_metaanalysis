...
for mclass in adata.obs.majorclass.cat.categories:
    ...
    for maturity in temp.obs.maturityclass.cat.categories:
        ...
        sc.tl.rank_genes_groups(temp2, 'source', pts=True)

        for j in temp2.obs.source.cat.categories:
            d = pd.DataFrame() 
            ...
            pd.DataFrame.to_csv(d, f'/storage/singlecell/jeanl/organoid/csv/reannotation/majorclass_degs/degs_by_source/{j}_{mclass}_{maturity}_deg.csv')

        sc.pl.rank_genes_groups_dotplot(temp2, title=f'{mclass} {maturity}', n_genes=10, save=f'{mclass}_{maturity}_deg.png')